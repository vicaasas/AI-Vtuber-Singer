...
bh, ah = signal.butter(N=5, Wn=48, btype="high", fs=16000) #建立一個高通濾波器
...
class VC(object):
    ...
    def get_f0(
        ...
        f0_coarse = np.rint(f0_mel).astype(int) # 需要修改
    ...
    def pipeline(
        ...
        if (
            ...
            try:
                index = faiss.read_index(file_index) # 從index檔案中讀出資料
            ...
            index = big_npy = None
        audio = signal.filtfilt(bh, ah, audio) #套用高通濾波器，去除低頻雜訊
        ...
        t = None
        t1 = ttime()
        audio_pad = np.pad(audio, (self.t_pad, self.t_pad), mode="reflect") # 跟507行padding的長度不一樣
        ...
        if hasattr(f0_file, "name") == True:
            try:
                ...
                traceback.print_exc()
        sid = torch.tensor(sid, device=self.device).unsqueeze(0).long() #說話者id
        ...
        if if_f0 == 1:
            ...
            pitch = pitch[:p_len]
            pitchf = pitchf[:p_len]
            if self.device == "mps": # mac 需要轉為float32
        ...
        for t in opt_ts:
            ...
            s = t
        if if_f0 == 1: # 把opt_ts所有時間段做完之後，剩下的部分
            audio_opt.append(
                self.vc(
                    model,
                    net_g,
        ...
        if rms_mix_rate != 1:
            audio_opt = change_rms(audio, 16000, audio_opt, tgt_sr, rms_mix_rate) # 調整音量變化要接近輸入音頻還是轉換後的音頻
